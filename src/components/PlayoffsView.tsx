...
function getTeamSecondaryColor(teamId: number): string {
  return NBA_TEAMS.find(t => t.id === teamId)?.secondaryColor ?? '#666'
}
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex-1 space-y-2">
            ...
            <div className="flex items-center gap-2">
              <div
                ...
                style={{ backgroundColor: getTeamColor(series.team1Id) }}
              >
                {series.team1Abbr}
              </div>
              <span className={`text-sm font-semibold flex-1 ${isComplete && series.winnerId === series.team1Id ? '' : isComplete ? 'text-muted-foreground' : ''}`}
                style={{ fontFamily: 'Space Grotesk, sans-serif' }}>
                {series.team1Name}
              </span>
              <span className={`text-lg font-bold tabular-nums ${team1Leading ? 'text-foreground' : 'text-muted-foreground'}`}>
                {series.team1Wins}
              </span>
            </div>
            ...
            <div className="flex items-center gap-2">
              <div
                className="w-7 h-7 rounded-lg flex items-center justify-center text-white text-[10px] font-bold shrink-0"
                style={{ backgroundColor: getTeamColor(series.team2Id) }}
              >
                {series.team2Abbr}
              </div>
              <span className={`text-sm font-semibold flex-1 ${isComplete && series.winnerId === series.team2Id ? '' : isComplete ? 'text-muted-foreground' : ''}`}
                style={{ fontFamily: 'Space Grotesk, sans-serif' }}>
                {series.team2Name}
              </span>
              <span className={`text-lg font-bold tabular-nums ${team2Leading ? 'text-foreground' : 'text-muted-foreground'}`}>
                {series.team2Wins}
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center gap-1 ml-3">
            ...
            {!isComplete && !hasGameToday && (
              <span className="text-[10px] text-muted-foreground uppercase font-semibold">
                {isTied ? 'Tied' : team1Leading ? `${series.team1Abbr} leads` : `${series.team2Abbr} leads`}
              </span>
            )}
            ...
            return (
              <div key={game.id} className={`flex items-center justify-between px-4 py-2 text-xs border-b border-border/20 last:border-0 ${isToday && !isFinal ? 'bg-green-500/5' : ''}`}>
                ...
                <div className="flex-1 flex items-center justify-center gap-2">
                  <span className={`font-semibold ${visitorWon ? '' : isFinal ? 'text-muted-foreground' : ''}`}>
                  ...
                  {isFinal ? (
                    ...
                    <span className="text-green-500 font-bold">TONIGHT</span>
                  ) : (
                    <span className="text-muted-foreground">vs</span>
                  )}
                  <span className={`font-semibold ${homeWon ? '' : isFinal ? 'text-muted-foreground' : ''}`}>
                    {game.home_team.abbreviation}
                  </span>
                </div>
                <span className="text-muted-foreground w-16 text-right">
                  {isFinal ? 'Final' : isToday ? 'Live' : game.date?.slice(5, 10) ?? ''}
                </span>
              </div>
            );
          })}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div>
          ...
          <div className="space-y-3">
            {westSeries.map(s => (
              <SeriesCard key={`${s.team1Id}-${s.team2Id}`} series={s} />
            ))}
            {westSeries.length === 0 && (
              <p className="text-sm text-muted-foreground py-4">No Western Conference playoff games</p>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            ...
            {eastSeries.length === 0 && (
              <p className="text-sm text-muted-foreground py-4">No Eastern Conference playoff games</p>
            )}
          </div>
        </div>
      </div>
